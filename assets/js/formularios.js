...
let add = 1;
var comprar = d.getElementById("comprar"), cancelar = d.getElementById("cancelar"), compra_online = d.getElementById("compra_online");
comprar.disabled=true, cancelar.disabled=true, compra_online.disabled=true;
comprar.classList.add("deshabilitar"), cancelar.classList.add("deshabilitar"), compra_online.classList.add("deshabilitar");
    ...
    formulario.addEventListener("submit", (e) => {
        ...
        if(campos.nombre_prov, campos.producto, campos.cantidad_prov,campos.pcompra_prov,campos.pventa_prov){
            ...
            for (var i = 0, row; row = table_body.rows[i]; i++) {
                if(table_body.rows[i].cells[0].innerText === formulario.producto_prov.value){
            ...
            comprar.disabled=false, cancelar.disabled=false, compra_online.disabled=false;
            comprar.classList.remove("deshabilitar"), cancelar.classList.remove("deshabilitar"), compra_online.classList.remove("deshabilitar");
            // ----------------
